<?php
/**
 * The complete management for the Insightly-CF7 feeds custom post type.
 *
 * @since      1.0.0
 ...
 * @author     MakeWebBetter <https://makewebbetter.com>
 */
class Mwb_Cf7_Integration_Insightly_Feed_Module {
	...
	/**
	 * Main Mwb_Cf7_Integration_Insightly_Feed_Module Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_Insightly_Feed_Module is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Cf7_Integration_Insightly_Feed_Module - Main instance.
	 */
	...
	public function register_feeds_post_type() {
		...
		$labels = array(
			'name'               => _x( 'Feeds', 'Post Type General Name', 'mwb-cf7-integration-with-insightly' ),
			'singular_name'      => _x( 'Feed', 'Post Type Singular Name', 'mwb-cf7-integration-with-insightly' ),
			'menu_name'          => _x( 'Crm Feeds', 'Admin menu name', 'mwb-cf7-integration-with-insightly' ),
			'parent_item_colon'  => __( 'Parent Feed', 'mwb-cf7-integration-with-insightly' ),
			'all_items'          => __( 'All Feeds', 'mwb-cf7-integration-with-insightly' ),
			'view_item'          => __( 'View Feed', 'mwb-cf7-integration-with-insightly' ),
			'add_new_item'       => __( 'Add New Feed', 'mwb-cf7-integration-with-insightly' ),
			'add_new'            => __( 'Add New', 'mwb-cf7-integration-with-insightly' ),
			'edit_item'          => __( 'Edit Feed', 'mwb-cf7-integration-with-insightly' ),
			'update_item'        => __( 'Update Feed', 'mwb-cf7-integration-with-insightly' ),
			'search_items'       => __( 'Search Feed', 'mwb-cf7-integration-with-insightly' ),
			'not_found'          => __( 'Not Found', 'mwb-cf7-integration-with-insightly' ),
			'not_found_in_trash' => __( 'Not found in Trash', 'mwb-cf7-integration-with-insightly' ),
		);

		$args = array(
			'label'                => __( 'Feeds', 'mwb-cf7-integration-with-insightly' ),
			'description'          => __( 'Feeds for crm', 'mwb-cf7-integration-with-insightly' ),
			'labels'               => $labels,
	...
	public function add_feed_meta_box() {
		...
		$metaboxes[] = array(
			...
			'title'    => esc_html__( 'Feed details', 'mwb-cf7-integration-with-insightly' ),
		...
		if ( ! $dependent ) {
			$metaboxes[] = array(
				...
				'title'    => esc_html__( 'Conditional Statements', 'mwb-cf7-integration-with-insightly' ),
	...
	public function get_pro_callback( $key = '' ) {
		...
		switch ( $key ) {
			...
			case 'pro-associate-category':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_associate_category' );
				break;

			case 'pro-associate-project':
				$result = array( $pro_class, 'mwb_cf7_int_pro_feeds_associate_project' );
				break;
