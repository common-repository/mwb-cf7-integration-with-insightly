 ...
 * @subpackage Mwb_Cf7_Integration_With_Insightly/public
 */
 ...
 */
class Mwb_Cf7_Integration_With_Insightly_Public {
	...
	/**
	 * Main Mwb_Cf7_Integration_With_Insightly_Crm_Public Instance.
	 *
	 * Ensures only one instance of Mwb_Cf7_Integration_With_Insightly_Crm_Public is loaded or can be loaded.
	 *
	 * @since 1.0.0
	 *
	 * @static
	 * @return Mwb_Cf7_Integration_With_Insightly_Crm_Public - Main instance.
	 */
	public static function get_instance() {

		if ( null == self::$_instance ) { //phpcs:ignore

			self::$_instance = new self( 'mwb-cf7-integration-with-insightly', MWB_CF7_INTEGRATION_WITH_INSIGHTLY_VERSION );
		}
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/mwb-cf7-integration-with-insightly-public.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/mwb-cf7-integration-with-insightly-public.js', array( 'jquery' ), $this->version, false );
	...
	public function mwb_cf7_integration_check_error_for_mail( $response = array(), $data = array(), $log = array(), $request = array() ) {
		...
		if ( isset( $response['code'] ) ) { // phpcs:ignore
			...
			$module       = $log['insightly_object'];
			$response_obj = $api::get_id_param_with_object( $module );
			$response_id  = isset( $response[ $response_obj ] ) ? $response[ $response_obj ] : '';
	...
	public function mwb_cf7_integration_send_email( $info, $data ) {
		...
		if ( ! empty( $info ) && is_array( $info ) ) {
			...
			$subject   = sprintf( esc_html__( 'Error While Posting CF7 form data over %s ', 'mwb-cf7-integration-with-insightly' ), esc_html( $this->crm_name ) );
			...
			if ( isset( $info['data'] ) && isset( $info['data'] ) ) {
				$info_msg    = $info['data'][0]['MESSAGE'];
				$info_detail = is_array( $info['data'] ) ? wp_json_encode( $info['data'] ) : $info['data'];
			}

			$email_data = array(
				'Title'          => esc_html__( 'MWB CF7 Integration with Insightly', 'mwb-cf7-integration-with-insightly' ),
